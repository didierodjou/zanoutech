...
export class DashboardService {
  ...
  async getStats() {
    const [students, teachers, classes, staff, subjects, pendingSalaries] = await Promise.all([
      this.prisma.student.count(),
      this.prisma.teacher.count(),
      this.prisma.class.count(),
      this.prisma.staff.count(),
      this.prisma.subject.count(),
      this.prisma.salary.count({ where: { isPaid: false } }),
    ]);

    // ✅ mainClassId n'existe pas dans TeacherWhereInput — filtrer via la relation
    const principals = await this.prisma.teacher.count({
      where: {
        mainClass: { isNot: null }, // ✅
      },
    });
  ...
  async getPrincipalsCount() {
    // ✅ mainClass: { isNot: null } au lieu de mainClassId: { not: null }
    return this.prisma.teacher.count({
      where: { mainClass: { isNot: null } },
    });
  }

  async getPendingSalariesCount() {
    return this.prisma.salary.count({ where: { isPaid: false } });
  }
}